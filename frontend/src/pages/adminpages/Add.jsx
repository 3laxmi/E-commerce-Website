...
const Add = () => {
  ...
  const {backendUrl, currency, token, navigate, getProductsData} = useContext(ShopContext)

  useEffect(() => {
    // Check if there's a product to edit
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Refresh products on user side
        console.log('Calling getProductsData to refresh products')
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <button type="submit" className='w-28 py-3 mt-4 bg-black text-white'>{editingProductId ? 'UPDATE' : 'ADD'}</button>
    </form>
  )
}
